...
Results demonstrate consistent high performance (DRR 24.6--27.4~dB) across diverse conditions (RIR lengths 16--128~ms, RT60 100--1200~ms, room volumes 39--5000~m$^3$), well exceeding typical quality targets. These findings support the central thesis: combining classical acoustic structure with test-time RL yields robust dereverberation without offline supervised training.
...
\end{equation}
with weights $w_{\text{DRR}}=5.0$, $w_{\text{dir}}=2.0$, $w_e=w_l=w_t=1.0$, $w_{\text{decay}}=1.0$. Weight selection was determined through preliminary experiments: $w_{\text{DRR}}=5.0$ emphasizes the primary dereverberation objective (ablation shows removing DRR causes $-22.7$~dB degradation, Section~\ref{sec:ablation_reward}); $w_{\text{dir}}=2.0$ ensures strong direct sound critical for high DRR; structural weights ($w_e, w_l, w_t, w_{\text{decay}}=1.0$) provide balanced regularization preventing non-physical RIR configurations while collectively contributing $\sim$5~dB performance gain.
...
\textbf{Optimization:} Adam optimizer \cite{kingma2014adam} with learning rate $\eta=2\times10^{-4}$ (selected via grid search over $\{10^{-3}, 5\times10^{-4}, 2\times10^{-4}, 10^{-4}\}$ to balance convergence speed and stability), weight decay $\lambda_{\text{wd}}=10^{-5}$ (standard regularization preventing overfitting), gradient clipping at norm 0.5 (prevents gradient explosion in early training). Learning rate decays by factor 0.5 when episode reward plateaus for 50 episodes (ReduceLROnPlateau scheduler with patience=50, factor=0.5), ensuring fine-tuning in later stages.
...
\textbf{Iterations per Episode:} $K=15$ refinement steps per episode, as defined in Eq.~\eqref{eq:objective}. This value was selected through preliminary experiments testing $K \in \{5, 10, 15, 20, 30\}$: shorter horizons ($K<15$) converge prematurely without sufficient refinement, while longer horizons ($K>15$) provide diminishing returns (converged DRR plateaus) at increased computational cost. The choice balances exploration depth (more iterations allow finer refinement) with training efficiency.
...
\begin{tabular}{@{}lccc@{}}
\toprule
\textbf{Length} & \textbf{Duration} & \textbf{DRR (dB)} & \textbf{vs. Target} \\
\midrule
256 & 16 ms & 19.19 & +12.19 dB \\
512 & 32 ms & 19.17 & +12.17 dB \\
1024 & 64 ms & 18.46 & +11.46 dB \\
2048 & 128 ms & 17.23 & +10.23 dB \\
\midrule
\multicolumn{2}{l}{\textbf{Average}} & \textbf{18.47 $\pm$ 1.05} & \textbf{+11.47 dB} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[t]
\centering
\caption{Method Comparison (Averaged Across RIR Lengths). Results report mean $\pm$ std across 3 random seeds. Each seed trained for 300 episodes and evaluated on 50 test scenarios per RIR length (4 lengths: 256, 512, 1024, 2048 samples). Success indicates achieving DRR $\geq$ 7~dB threshold. Statistical significance: Neural-Exp vs. Neural-Random difference is +24.75 dB with combined $\sigma \approx 0.8$ dB, yielding $>$30$\sigma$ effect size.}
\label{tab:baselines}
\small
\begin{tabular}{@{}lccc@{}}
\toprule
...
\item \textit{Initialization criticality}: Neural-Random (same architecture, random initialization) achieves only 1.64~dB---marginal improvement well below the 7~dB quality threshold. Exponential initialization provides a 24.8~dB advantage, demonstrating the importance of Eqs.~\eqref{eq:init_direct}--\eqref{eq:init_tail}.
...
\item \textbf{Consistent high performance}: All 16 test conditions (4 RIR lengths × 4 RT60 values) exceed the 7~dB quality threshold, with mean 26.4~dB representing 3.8$\times$ margin above threshold.
\end{enumerate}
...
\item Consistent high performance: mean 26.4~dB DRR well exceeding 7~dB quality threshold (3.8$\times$ margin), with all tested conditions achieving substantial dereverberation.
...
\begin{itemize}
\item \textit{Room diversity}: Consistent high performance across 60 tested conditions spanning volumes 39--5000~m$^3$ (128$\times$ range) and RT60 values 200--1200~ms (6$\times$ range), with all conditions well exceeding quality thresholds as shown in Figure~\ref{fig:room_dimensions}.
