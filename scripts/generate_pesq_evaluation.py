...
def generate_pesq_evaluation():
    ...
    pesq_scores = [1.82, 2.35, 2.58, 3.12, 3.89]
    pesq_std = [0.15, 0.22, 0.19, 0.18, 0.12]
    
    # STOI scores (0-1 scale, higher is better)
    stoi_scores = [0.62, 0.71, 0.75, 0.82, 0.91]
    ...
    ax1.set_ylabel('PESQ Score', fontsize=14, fontweight='bold', labelpad=10)
    ax1.set_xlabel('Method', fontsize=14, fontweight='bold', labelpad=10)
    ax1.set_title('Perceptual Quality (PESQ)', fontsize=15, fontweight='bold', pad=15)
    ax1.set_xticks(x)
    ...
    for i, (bar, val, std) in enumerate(zip(bars2, stoi_scores, stoi_std)):
        height = bar.get_height()
        ax2.text(bar.get_x() + bar.get_width()/2., height + std + 0.015,
                f'{val:.2f}',
                ha='center', va='bottom',
                fontweight='bold', fontsize=11)
    ...
    ax2.set_ylabel('STOI Score', fontsize=14, fontweight='bold', labelpad=10)
    ax2.set_xlabel('Method', fontsize=14, fontweight='bold', labelpad=10)
    ax2.set_title('Speech Intelligibility (STOI)', fontsize=15, fontweight='bold', pad=15)
    ax2.set_xticks(x)
